...
from xafs.extract_Feff_phase import interpolation
import numpy as np
...
class fit():
    ...
    def set_fit_range(self,start, end):
        self.k_fit = np.linspace(start, end, (end - start) * 40)
        return
    ...
    def set_shells(self,shells):
        self.shells = shells
        self.bounds = np.vstack(([0]*len(shells)*3+[-10],[12]*(len(shells)*3+1))).T
