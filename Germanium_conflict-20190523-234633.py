...
def EXAFS_shell(x,N,R,ss,e):
    return N * (x+e) **weight \
           * abs( Feff1.astype(float)) / ((x.astype(float)+e) * R ** 2) \
           * np.sin(2 * (x.astype(float)+e) * R +  phase1.astype(float)) \
           * np.exp(-2 * R /  lambda1.astype(float)) \
           * np.exp(-2 * ss * (x.astype(float)+e) ** 2)

plt.figure()
plt.plot(k,EXAFS_shell(k,3.324,2.447,0.00236,1.5),label='path')
...
bounds_dic = {'Ge1': np.array([(3, 4), (2.1, 3), (0.0028, 0.0029)]),
              'Ge2': np.array([(9,12), (3., 4.3), (0.0038, 0.0039)]),
              'Ge3': np.array([(9,12), (4.3, 5.2), (0.0037, 0.0038)]),
              'e': np.array([(-6,6)]),
              'sd': np.array([(0,1)])}
